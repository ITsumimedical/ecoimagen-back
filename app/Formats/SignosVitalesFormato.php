...
class SignosVitalesFormato extends Fpdf
{
    public static $signoVital;
    ...
    {
        $idSignoVital = $datos['id'];

        self::$signoVital = SignosVitales::with('usuario')->find($idSignoVital);

        self::$admision = AdmisionesUrgencia::with('afiliado')->find(self::$signoVital->admision_urgencia_id);

        self::$operdaor = User::find(self::$signoVital->created_by);

        self::$consulta = Consulta::where('admision_urgencia_id', self::$signoVital->admision_urgencia_id)->first();

        self::$cie10 = Cie10Afiliado::where('consulta_id', self::$consulta->id)->where('esprimario', true)->with('cie10')->get();

        $pdf = new SignosVitalesFormato('p', 'mm', 'Letter');
        $pdf->AliasNbPages();
        $pdf->AddPage();
        $this->body($pdf, $datos);
        $pdf->Output();
    ...
    public function Header()
    {
        if ($this->page == 1) {
            ...
            $this->SetXY(90, 5);
            $this->Cell(0, 5, utf8_decode('Signos Vitales'), 0, 1, 'L');
    ...
    public function body($pdf, $data)
    {
        ...
        $pdf->SetXY(30, 45);
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(0, 5, utf8_decode(self::$signoVital->id), 0, 1, 'L');
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(55, 100);
        $pdf->Cell(0, 5, utf8_decode(' ' . Carbon::parse(self::$signoVital->fecha)->format('Y/m/d')), 0, 1, 'L');
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(125, 100);
        $pdf->Cell(0, 5, utf8_decode(' ' . (self::$signoVital->peso)), 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetXY(145, 100);
        $pdf->Cell(0, 5, utf8_decode('Tension Arterial: '), 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(175, 100);
        $pdf->Cell(0, 5, utf8_decode(' ' . (self::$signoVital->tension_arterial)), 0, 1, 'L');
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(55, 105);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$signoVital->frecuencia_respiratoria), 0, 1, 'L');
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(125, 105);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$signoVital->frecuencia_cardiaca), 0, 1, 'L');
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(55, 110);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$signoVital->temperatura . ' C°'), 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetXY(145, 110);
        $pdf->Cell(0, 5, utf8_decode('Saturacion Oxigeno: '), 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(175, 110);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$signoVital->saturacion_oxigeno . ' C°'), 0, 1, 'L');
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(125, 110);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$signoVital->glucometria), 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetXY(145, 105);
        $pdf->Cell(0, 5, utf8_decode('TAM: '), 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(175, 105);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$signoVital->tam), 0, 1, 'L');
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(55, 115);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$signoVital->usuario->operador->nombre_completo), 0, 1, 'L');
        ...
        if (!file_exists($logo)) {
            ...
            $pdf->Cell(0, 5, utf8_decode(' ' . self::$signoVital->usuario->operador->documento), 0, 1, 'L');
        } else {
