...
class HistoriaClinicaPrimeraInfancia extends Fpdf
{
    protected static $consulta;

    public function bodyPrimeraInfancias($pdf, $consulta)
    {
        ...
        if ($noTieneAntecedentesVacunales) {
            ...
            foreach ($antecedentesVacunales as $vacuna) {
                ...
                // Concatenar los tres campos
                $detalleVacuna = utf8_decode('VACUNA: ' . $vacunaNombre . ' | DOSIS: ' . $dosis . ' | FECHA DE DOSIS: ' . $fechaDosis . ' | REGISTRADO POR: ' . utf8_decode($nombreCompletoMedico));
        ...
        if ($antecedentesHospitalarios->isEmpty()) {
            $pdf->SetX(12);
            $pdf->Cell(186, 6, utf8_decode('No refiere'), 1, 0, 'C');
            $pdf->Ln();
        } else {
            $contador = 1;
        ...
        $pdf->Ln();
        // Obtén los hábitos alimentarios de la consulta actual
        $consultaId = self::$consulta->id;
        $habitosAlimentarios = HabitosAlimentarios::where('consulta_id', $consultaId)->get();
        ...
        if ($habitosAlimentarios->isNotEmpty()) {
            foreach ($habitosAlimentarios as $habito) {
                $pdf->SetX(12);

                $campos = [
                    "Lactando Actualmente" => $habito->lactando_actualmente ? "Sí" : "No refiere",
                    "Lactancia Materna Exclusiva" => $habito->lactancia_materna_exclusiva ? "Sí" : "No refiere",
                    "Postura Madre-Niño" => $habito->postura_madre_niño ?: "No refiere",
                    "Agarre" => $habito->agarre ?: "No refiere",
                    "Succión" => $habito->succion ?: "No refiere",
                    "Madre Reconoce Hambre/Saciedad Bebé" => $habito->madre_reconoce_hambre_saciedad_bebe ? "Sí" : "No refiere",
                    "Necesidades Madre Lactancia Materna" => $habito->necesidades_madre_lactancia_materna ?: "No refiere",
                    "Recibió Preparación Prenatal" => $habito->recibio_preparacion_prenatal ? "Sí" : "No refiere",
                    "Suministra Leche en Hospital Neonatal" => $habito->suministra_leche_hospitalario_neonatal ? "Sí" : "No refiere",
                    "Expectativas Madre/Familia" => $habito->expectativas_madre_familia ?: "No refiere",
                    "Frecuencia Lactancia" => $habito->frecuencia_lactancia ?: "No refiere",
                    "Duración Lactancia" => $habito->duracion_lactancia ?: "No refiere",
                    "Dificultades Lactancia Materna" => $habito->dificultades_lactancia_materna ?: "No refiere",
                    "Madre Extrae/Conserva Leche" => $habito->madre_extrae_conserva_leche ? "Sí" : "No refiere",
                    "Cómo Realiza Extracción/Conservación Leche" => $habito->como_realiza_extraccion_conservacion_leche ?: "No refiere",
                    "Alimentado con Leche de Fórmula" => $habito->alimentado_leche_formula ? "Sí" : "No refiere",
                    "Inicio de Alimentos/Agua/Otra Bebida" => $habito->inicio_alimentos_agua_otra_bebida ? "Sí" : "No refiere",
                    "Durante el Día Ayer Recibió Líquidos" => $habito->durante_dia_ayer_recibio_liquidos ? "Sí" : "No refiere",
                    "Durante el Día Recibió Leche" => $habito->durante_dia_recibio_leche ? "Sí" : "No refiere",
                    "Durante el Día Recibió Leche de Vaca" => $habito->durante_dia_recibio_leche_vaca ? "Sí" : "No refiere",
                    "Durante el Día Recibió Sopa" => $habito->durante_dia_recibio_sopa ? "Sí" : "No refiere",
                    "Edad en Meses de Diferentes Alimentos" => $habito->edad_meses_diferentes_alimentos ?: "No refiere",
                    "Consumo de Dieta Familiar" => $habito->consumo_dieta_familiar ? "Sí" : "No refiere",
                    "Cuántas Comidas al Día" => $habito->cuantas_comidas_dia ?: "No refiere",
                    "Consumo de 5 Porciones de Frutas" => $habito->consumo_5_porciones_frutas ? "Sí" : "No refiere",
                    "Dieta Balanceada/Baja en Azúcares" => $habito->dieta_balanceada_baja_azucares ? "Sí" : "No refiere",
                ];
        ...
        $pdf->SetX(12);
        $pdf->SetFont('arial', '', '8');
        $pdf->Cell(186, 4, utf8_decode("Resultado del cuestionario: ") . utf8_decode(isset(self::$consulta["cuestionarioVale"]["interpretacion_resultado"]) && self::$consulta["cuestionarioVale"]["interpretacion_resultado"] !== null ? self::$consulta["cuestionarioVale"]["interpretacion_resultado"] : 'sin hallazgos'), 1, 0, 'L');
        $pdf->Ln();
        ...
        if (isset($consulta['familiograma'])) {
        ...
        // $pdf->SetFont('Arial', '', 8);
        // $pdf->SetX(12);
        // if (self::$consulta["afiliado"]["edad_cumplida"] >= 0 && self::$consulta["afiliado"]["edad_cumplida"] <= 11) {
        //     $pdf->Cell(62, 6, utf8_decode('Motricidad gruesa: ' . (self::$consulta["HistoriaClinica"]["motricidad_gruesa"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Motricidad fina: ' . (self::$consulta["HistoriaClinica"]["motricidad_fina"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Audición y lenguaje: ' . (self::$consulta["HistoriaClinica"]["audicion_lenguaje"] ?? 'No Evaluado')), 1);
        //     $pdf->Ln();
        //     $pdf->SetX(12);
        //     $pdf->Cell(62, 6, utf8_decode('Personal-Social: ' . (self::$consulta["HistoriaClinica"]["personal_social"] ?? 'No Evaluado')), 1);
        // }
        // if (self::$consulta["afiliado"]["edad_cumplida"] >= 0 && self::$consulta["afiliado"]["edad_cumplida"] <= 17) {
        //     $pdf->Cell(62, 6, utf8_decode('Percepción de los cuidadores: ' . (self::$consulta["HistoriaClinica"]["cuidado"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Resultado Escala Abreviada de Desarrollo-3: ' . (self::$consulta["HistoriaClinica"]["escala_desarrollo"] ?? 'No Evaluado')), 1);
        //     $pdf->Ln();
        //     $pdf->SetX(12);
        //     $pdf->Cell(62, 6, utf8_decode('Rendimiento escolar: ' . (self::$consulta["HistoriaClinica"]["rendimiento_escolar"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Porposito de activadades: ' . (self::$consulta["HistoriaClinica"]["actividades"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Ejercen autocontrol : ' . (self::$consulta["HistoriaClinica"]["autocontrol"] ?? 'No Evaluado')), 1);
        //     $pdf->Ln();
        //     $pdf->SetX(12);
        //     $pdf->Cell(62, 6, utf8_decode('Exhiben comportamientos fiables: ' . (self::$consulta["HistoriaClinica"]["comportamientos"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Expresan autoeficacia positiva : ' . (self::$consulta["HistoriaClinica"]["auto_eficacia"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Demuestran independencia: ' . (self::$consulta["HistoriaClinica"]["independencia"] ?? 'No Evaluado')), 1);
        //     $pdf->Ln();
        //     $pdf->SetX(12);
        //     $pdf->Cell(62, 6, utf8_decode('Capacidad para resolver problemas: ' . (self::$consulta["HistoriaClinica"]["actividades_proposito"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Exhiben un locus de control interno : ' . (self::$consulta["HistoriaClinica"]["control_interno"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Funciones ejecutivas : ' . (self::$consulta["HistoriaClinica"]["funciones_ejecutivas"] ?? 'No Evaluado')), 1);
        //     $pdf->Ln();
        //     $pdf->SetX(12);
        //     $pdf->Cell(62, 6, utf8_decode('Identidad :' . (self::$consulta["HistoriaClinica"]["identidad"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Resultado de instrumento: ' . (self::$consulta["HistoriaClinica"]["valoracion_identidad"] ?? 'No Evaluado')), 1);
        //     $pdf->Cell(62, 6, utf8_decode('Autonomía: ' . (self::$consulta["HistoriaClinica"]["autonomia"] ?? 'No Evaluado')), 1);
        //     $pdf->Ln();
        //     $pdf->SetX(12);
        //     $pdf->Cell(62, 6, utf8_decode('Resultado de valoración de la autonomía :' . (self::$consulta["HistoriaClinica"]["valoracion_autonomia"] ?? 'No Evaluado')), 1);
        //     $pdf->Ln();
        // }
        // $pdf->Ln();
        ...
        // $remisionPrograma = $firstRemision->remision_programa ?? 'No evaluado';
        // $observacion = $firstRemision->observacion ?? 'No evaluado';
        // $pdf->SetX(12);
        // $pdf->SetFont('Arial', '', 8);
