...
class CertificadoTriage extends FPDF
{
    use PdfTrait;
    public static $certificadoTriage;
    // public static $afiliado;
    // public static $entidad;
    // public static $tipo;
    // public static $admision;

    public function generar($certificadoTriage)
    {
        self::$certificadoTriage = $certificadoTriage;
        $this->generarPDF('I');
    }
    ...
    public function Header()
    {
        ...
        $this->SetY(38);
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(280, 6, utf8_decode('CERTIFICADO DE ADMISIÓN'), 0, 0, 'C');
        $this->SetXY(12, 43);
        $this->Ln();
        $this->Cell(20, 10, utf8_decode('No ingreso: '), 0, 0, 'L');
        $this->SetFont('Arial', '', 10);
        $this->Cell(5, 10, utf8_decode(self::$certificadoTriage->id), 0, 0, 'L');
        $this->Ln();
        $this->SetY(62);
        $this->SetFont('Arial', '', 13);
        $this->MultiCell(280, 4, utf8_decode('Este formato evidencia el ingreso a la admisión del afiliado.' . self::$certificadoTriage->afiliado->nombre_completo . ' ' . 'identificado (a) con ' . self::$certificadoTriage->afiliado->tipoDocumento->sigla . '-' . self::$certificadoTriage->afiliado->numero_documento . ' en MEDICINA INTEGRAL S.A.S y para constancia queda registrado de este.'), 0, 'L', 0);

        // $this->Ln();
        // $this->SetFont('Arial', 'B', 13);
        // $this->Cell(31, 4, utf8_decode('Aseguradora:'), 0, 0, 'l');
        // $this->SetFont('Arial', '', 13);
        // $this->Cell(100, 4, utf8_decode(self::$entidad->nombre), 0, 0, 'l');
        $this->Ln();

        $this->SetFont('Arial', 'B', 13);
        $this->Cell(35, 4, utf8_decode('Fecha ingreso:'), 0, 0, 'l');
        $this->SetFont('Arial', '', 13);
        $this->Cell(100, 4, utf8_decode(self::$certificadoTriage->created_at), 0, 0, 'l');
        // $this->SetFont('Arial', 'B', 13);
    ...
    public function footer()
    {
        ...
        // Firma del paciente
        $base64Image = self::$certificadoTriage->firma_afiliado ?? null;
        if ($base64Image) {
        ...
        if (!empty(self::$certificadoTriage->firma_acompanante)) {
            $base64Image = self::$certificadoTriage->firma_acompanante;
            $explodedData = explode(',', $base64Image);
