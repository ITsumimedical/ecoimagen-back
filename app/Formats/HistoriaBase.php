...
class HistoriaBase extends Fpdf
{
    protected static $data;

    public function generar($datos,$correo)
    {

        self::$data=$datos;
        $pdf = new HistoriaBase('p', 'mm', 'A4');
        $pdf->AliasNbPages();
        $pdf->AddPage();
    ...
    public function bodyHistoriaClinica($pdf)
    {
        ...
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(46.5, 4, utf8_decode(self::$data["Ocupacion"] ? 'No Reporta':self::$data["Ocupacion"]), 1, 0, 'l');
        $pdf->SetXY(105,61);
        $pdf->SetFont('Arial', 'B', 8);
