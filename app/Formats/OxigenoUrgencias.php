...
class OxigenoUrgencias extends Fpdf
{
    public static $oxigeno;
    ...
    public function generar($datos)
    {
        ...
        self::$oxigeno = Oxigeno::with('usuario')->find($idOxigeno);

        self::$admision = AdmisionesUrgencia::with('afiliado')->find(self::$oxigeno->admision_urgencia_id);

        self::$operdaor = User::find(self::$oxigeno->created_by);

        self::$consulta = Consulta::where('admision_urgencia_id', self::$oxigeno->admision_urgencia_id)->first();

        self::$cie10 = Cie10Afiliado::where('consulta_id', self::$consulta->id)->where('esprimario', true)->with('cie10')->get();

        $pdf = new OxigenoUrgencias('p', 'mm', 'Letter');
        $pdf->AliasNbPages();
        $pdf->AddPage();
        $this->body($pdf, $datos);
        $pdf->Output();
    ...
    public function Header()
    {
        if ($this->page == 1) {
            ...
            $this->SetXY(90, 5);
            $this->Cell(0, 5, utf8_decode('Oxigeno'), 0, 1, 'L');
    ...
    public function body($pdf, $data)
    {
        ...
        $pdf->SetXY(30, 45);
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(0, 5, utf8_decode(self::$oxigeno->id), 0, 1, 'L');
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(55, 100);
        $pdf->Cell(0, 5, utf8_decode(' ' . Carbon::parse(self::$oxigeno->fecha)->format('Y/m/d')), 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetXY(85, 100);
        $pdf->Cell(0, 5, utf8_decode('Hora Inicio: '), 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(125, 100);
        $pdf->Cell(0, 5, utf8_decode(' ' . Carbon::parse(self::$oxigeno->hora_inicio)->format('H:i')), 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetXY(145, 100);
        $pdf->Cell(0, 5, utf8_decode('Hora Final: '), 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(165, 100);
        $pdf->Cell(0, 5, utf8_decode(' ' . Carbon::parse(self::$oxigeno->hora_final)->format('H:i')), 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetXY(15, 105);
        $pdf->Cell(0, 5, utf8_decode('Flujo: '), 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(55, 105);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$oxigeno->flujo), 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetXY(85, 105);
        $pdf->Cell(0, 5, utf8_decode('Total Litros: '), 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(125, 105);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$oxigeno->total_litros), 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetXY(15, 110);
        $pdf->Cell(0, 5, utf8_decode('Total Horas: '), 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(55, 110);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$oxigeno->total_horas), 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetXY(85, 110);
        $pdf->Cell(0, 5, utf8_decode('Modo Administracion: '), 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(125, 110);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$oxigeno->modo_administracion), 0, 1, 'L');
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetXY(55, 115);
        $pdf->Cell(0, 5, utf8_decode(' ' . self::$oxigeno->usuario->operador->nombre_completo), 0, 1, 'L');
        ...
        if (!file_exists($logo)) {
            ...
            $pdf->Cell(0, 5, utf8_decode(' ' . self::$oxigeno->usuario->operador->documento), 0, 1, 'L');
        } else {
