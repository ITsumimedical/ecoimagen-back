...
class CodigoPropio extends Fpdf
{
    var $angle = 0;
    private static $data;
    ...
    public function header()
    {
        ...
        if (self::$consulta->tipo_consulta_id == 1) {
            ...
            $this->Cell($anchoCelda, 0, utf8_decode('FECHA DE AUTORIZACIÓN: ' . self::$data["servicios"][0]['fecha_vigencia']), 0, 0, 'L');
            ...
            $this->Cell($anchoCelda, 0, utf8_decode($sedeProveedor->nombre), 0, 0, 'L');
            //Quinta linea
    ...
    {
        $servicios = OrdenCodigoPropio::leftJoin('auditorias', 'auditorias.orden_codigo_propio_id', 'orden_codigo_propios.id')
        ->whereIn('orden_codigo_propios.estado_id', [1, 4, 14])
        ...
        foreach ($serviciosAgrupados as $rep_id => $servicios_rep) {
            // Buscamos el rep y asignamos el nombre y dirección a cada hoja
            ...
            if ($rep) {
                ...
                // Si el rep es null, valores por defecto
            ...
            // Nueva página para cada grupo de servicios_rep
            $pdf->AddPage();
            ...
            foreach ($servicios_rep as $servicio) {
                ...
                $pdf->SetXY(5, $Y);
                $pdf->MultiCell(15, 4, $servicio->codigoPropio->codigo ?? 'N/A', 0, 'C', 0);
                $pdf->SetXY(24, $Y);
                $pdf->MultiCell(150, 4, utf8_decode(strtoupper($servicio->codigoPropio->nombre ?? 'N/A')), 0, 'L', 0);

                $YN1 = round($pdf->GetY(), 0, PHP_ROUND_HALF_DOWN);
                $YN2 = round($pdf->GetY(), 0, PHP_ROUND_HALF_DOWN);

                $pdf->SetXY(188, $Y);
                $pdf->MultiCell(20, 4, $servicio->cantidad ?? 'N/A', 0, 'C', 0);
