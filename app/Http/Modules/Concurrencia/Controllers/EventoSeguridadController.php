...
use App\Http\Modules\Concurrencia\Repositories\EventoSeguridadRepository;
use App\Http\Modules\Concurrencia\Requests\CrearEventoSeguridadRequest;

class EventoSeguridadController extends Controller
{
    public function __construct(protected EventoSeguridadRepository $eventoRepository)
    {
    }

    public function guardarEvento(CrearEventoSeguridadRequest $request) {
        try {
            $evento = $this->eventoRepository->guardarEvento($request);
            return response()->json($evento, Response::HTTP_CREATED);
        } catch (\Throwable $th) {
            return response()->json([
                'mensaje' => 'Error al registrar el evento de seguridad',
                'error' => $th->getMessage()
